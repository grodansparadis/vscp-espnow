...
esp_err_t
espnow_log_get_config(espnow_log_config_t *config)
{
...
esp_err_t
espnow_log_set_config(const espnow_log_config_t *config)
{
  ESP_PARAM_CHECK(config);
  ESP_ERROR_RETURN(!g_log_init_flag, ESP_ERR_NOT_SUPPORTED, "log debugging is not initialized");
...
espnow_log_send_task(void *arg)
{
  ...
  for (; g_log_config;) {
    ...
    if (strcasecmp(log_info->tag, "espnow") && log_info->level <= g_log_config->log_level_espnow) {
      ...
      espnow_send(ESPNOW_DATA_TYPE_DEBUG_LOG,
    ...
    if (log_info->level <= g_log_config->log_level_custom) {
      if (g_log_config->log_custom_write) {
        g_log_config->log_custom_write(log_info->data,
                                       log_info->size,
                                       log_info->tag,
                                       log_info->level); /**< Write log data to custom */
      }
    }
...
void
__real_esp_log_writev(esp_log_level_t level, const char *tag, const char *format, va_list args);

void
__wrap_esp_log_writev(esp_log_level_t level, const char *tag, const char *format, va_list args)
{
  if (!g_log_config || level <= g_log_config->log_level_uart) {
    __real_esp_log_writev(level, tag, format, args);
  }

  if (!g_log_config || (level > g_log_config->log_level_espnow && level > g_log_config->log_level_flash &&
...
void
__wrap_esp_log_write(esp_log_level_t level, const char *tag, const char *format, ...)
{
  va_list list;
...
esp_err_t
espnow_log_init(const espnow_log_config_t *config)
{
  if (g_log_init_flag) {
    return ESP_FAIL;
  }
  ...
  xTaskCreatePinnedToCore(espnow_log_send_task,
                          "espnow_log_send",
                          3 * 1024,
                          NULL,
                          CONFIG_DEBUG_TASK_DEFAULT_PRIOTY,
                          NULL,
                          CONFIG_DEBUG_TASK_PINNED_TO_CORE);
