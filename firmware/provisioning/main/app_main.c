...
static esp_err_t app_espnow_prov_recv_cb(uint8_t *src_addr, void *data,
   ...
   */
  ESP_LOGI(TAG,
           "MAC: " MACSTR ", Channel: %d, RSSI: %d, Product_id: %s, Device "
                          "Name: %s, Auth Mode: %d, device_secret: %s",
...
static esp_err_t app_espnow_prov_recv_cb(uint8_t *src_addr, void *data,
  ...
  ESP_LOGI(TAG,
           "MAC: " MACSTR ", Channel: %d, RSSI: %d, wifi_mode: %d, ssid: %s, "
                          "password: %s, token: %s",
           MAC2STR(src_addr), rx_ctrl->channel, rx_ctrl->rssi,
           wifi_config->mode, wifi_config->sta.ssid, wifi_config->sta.password,
           wifi_config->token);

  ESP_ERROR_CHECK(
      esp_wifi_set_config(ESP_IF_WIFI_STA, (wifi_config_t *)&wifi_config->sta));
...
static esp_err_t app_initiator_init() {
  ...
  for (;;) {
    ...
    ESP_LOGI(TAG,
             "MAC: " MACSTR
             ", Channel: %d, RSSI: %d, Product_id: %s, Device Name: %s",
...
void app_main() {
  ...
  uint8_t key[16] = {0xA4, 0xA8, 0x6F, 0x7D, 0x7E, 0x11, 0x9B, 0xA3,
                    0xF0, 0xCD, 0x06, 0x88, 0x1E, 0x37, 0x1B, 0x98};

  memcpy(espnow_config.pmk, key, 16);
  espnow_config.qsize = 32; // CONFIG_APP_ESPNOW_QUEUE_SIZE;
