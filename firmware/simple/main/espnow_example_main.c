...
static void
example_espnow_deinit(example_espnow_send_param_t *send_param);
...
static void
example_espnow_recv_cb(const esp_now_recv_info_t *recv_info, const uint8_t *data, int len)
{
  example_espnow_event_t evt;
...
int
example_espnow_data_parse(uint8_t *data, uint16_t data_len, uint8_t *state, uint16_t *seq, uint32_t *magic)
{
...
void
example_espnow_data_prepare(example_espnow_send_param_t *send_param)
{
...
static void
example_espnow_task(void *pvParameter)
{
  example_espnow_event_t evt;
    ...
    switch (evt.id) {
      ...
      case EXAMPLE_ESPNOW_RECV_CB: {
        ...
        if (ret == EXAMPLE_ESPNOW_DATA_BROADCAST) {
           ...
           */
          // if (recv_state == 1) {
          //     /* The device which has the bigger magic number sends ESPNOW
          //     data, the other one
          //      * receives ESPNOW data.
          //      */
          //     if (send_param->unicast == false && send_param->magic >=
          //     recv_magic) {
          ...
          //         ESP_NOW_ETH_ALEN); example_espnow_data_prepare(send_param);
